...
export const createPaymentOrder = async (orderData) => {
    try {
        const response = await axios.post(`${API_BASE_URL}/guest/create-order`, orderData);
...
export const verifyPaymentAndCompleteRegistration = async (paymentData) => {
    try {
        const response = await axios.post(`${API_BASE_URL}/guest/verify-payment`, paymentData);
        return { success: true, ...response.data };
    } catch (error) {
        console.error('Payment verification and registration error:', error);
        const errorMessage = error.response?.data?.error || 'Payment verification and registration failed';
...
export const verifyPayment = async (paymentData) => {
    console.warn('verifyPayment is deprecated. Use verifyPaymentAndCompleteRegistration instead.');
